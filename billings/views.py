...
async def generate_chapa_payment_link(request, invoice_id: int):
    """
    Generate a Chapa payment link for an invoice.
    """
    invoice = get_object_or_404(Invoice, id=invoice_id, status="pending")
    ...
    payload = {
        ...
        "phone_number": "0912345678",  # Replace with actual user phone number
        "tx_ref": tx_ref,
...
async def confirm_payment(request, invoice_id: int):
    ...
    if response.status_code == 200 and data["status"] == "success":
        ...
        # Notify the cashier to approve payment
